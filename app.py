# app.py (VERSI LENGKAP DENGAN BONUS 1 & 2)
...
try:
    # 1. Pastikan build_html_report diimpor
    from tracker import RekapKelas, build_markdown_report, save_text, build_html_report
except ImportError:
    print("Error: Pastikan file __init__.py ada di dalam folder 'tracker' dan sudah diperbarui.")
    sys.exit(1)
...
# --- Fungsi-Fungsi Helper CSV (Sudah benar dari kode Anda) ---

def append_to_grades_csv(nim, nama):
    """Menambahkan data mahasiswa baru ke grades.csv dengan nilai default 0."""
    new_data = [nim, nama, 0, 0, 0, 0] # Sesuai format: nim,nama,quiz,tugas,uts,uas
...
def append_to_attendance_csv(nim, hadir_persen=0.0):
    ...
    new_data = [nim, hadir_persen] # Sesuai format: nim,hadir_persen
...
# GANTI FUNGSI tampilkan_menu LAMA ANDA DENGAN YANG INI:

def tampilkan_menu():
    """Tampilkan menu utama CLI (versi normal tanpa label bonus)."""
    menu_text = """
...
[bold]7)[/bold] Lihat rekap (Nilai < 70)
[bold]8)[/bold] Simpan laporan HTML
[bold]9)[/bold] Keluar
"""
    print(Panel(
        menu_text, 
...
def simpan_laporan_html():
    ...
    save_text("out/report.html", content) # <-- Simpan sebagai report.html

# --- FUNGSI MAIN (LOOP UTAMA) ---
...
def main():
    ...
    while True:
        ...
        try:
            ...
            # --- INI BAGIAN LOOP YANG DIPERBARUI ---
            elif pilihan == '7':
                lihat_rekap_filter_cli() # <-- Bonus 1
            elif pilihan == '8':
                simpan_laporan_html() # <-- Bonus 2
            elif pilihan == '9': # <-- Keluar sekarang jadi 9
            # --- AKHIR BAGIAN YANG DIPERBARUI ---
                
                print("[bold cyan]Terima kasih telah menggunakan Student Performance Tracker![/bold cyan]")
                break
            else:
                print("[red]Pilihan tidak valid. Harap masukkan angka 1-9.[/red]") # <-- Ganti jadi 1-9
        ...
        # Beri jeda sebelum menampilkan menu lagi
        if pilihan != '9': # <-- Ganti jadi 9
            try:
